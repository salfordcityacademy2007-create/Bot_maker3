...
// Jalali helper fallback. The bot only uses it for timestamps.
...
function deletefolder($path) {
    ...
    foreach ($items as $item) {
        ...
        if (is_dir($p)) deletefolder($p); else @unlink($p);
    }
    return @rmdir($path);
}

function Spam($id) { return true; }

function SendMessage($chat_id, $text, $parse_mode = 'html', $disable_web_page_preview = 'true') {
...
function init_storage() {
    $dirs = ['settings', 'usersData', 'botsData'];
    foreach ($dirs as $d) if (!is_dir($d)) @mkdir($d, 0777, true);
    $defaults = [
        'settings/onof.txt' => 'on',
        'settings/chnnl11.txt' => '',
        'settings/chnnl22.txt' => '',
        'settings/refral.txt' => '2',
        'MMBER.txt' => '',
        'usersData/Member.txt' => '',
        'usersData/blocklist.txt' => '',
    ];
    foreach ($defaults as $file => $value) if (!file_exists($file)) @file_put_contents($file, $value);
}
init_storage();

// Values that used to come from config.php.
$channel = 'IRA_Team';
$folder = '';
$botuser = '';
$me = bot('getMe');
if ($me && !empty($me->ok) && !empty($me->result)) {
